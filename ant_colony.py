...
class AntColony:
    class ant(Thread):
        ...
        def _pick_path(self):
            ...
            """
            # on the first pass (no pheromones), then we can just choice() to
            # find the next one
            if self.first_pass:
                import random
            ...
            # sum all attrativeness amounts for calculating probability of each
            # route in the next step
            for possible_next_location in self.possible_locations:
                # NOTE: do all calculations as float, otherwise we get integer
                # division at times for really hard to track down bugs
    ...
    def __init__(self, nodes, distance_callback, start=None, ant_count=50, alpha=.5, beta=1.2,
        ...
        # create matrix for master pheromone map, that records pheromone
        # amounts along routes
        self.pheromone_map = self._init_matrix(len(nodes))
        # create a matrix for ants to add their pheromones to, before adding
        # those to pheromone_map during the update_pheromone_map step
        ...
        # ant_count
        if not isinstance(ant_count, int):
    ...
    def _get_distance(self, start, end):
        ...
        if not self.distance_matrix[start][end]:
            ...
            if (not isinstance(distance, int)) and (
                    not isinstance(distance, float)):
                raise TypeError(
